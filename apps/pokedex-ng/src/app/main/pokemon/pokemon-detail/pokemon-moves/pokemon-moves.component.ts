    ...
    this.versionFilteredMoves.forEach(
      (move) =>
        (move.version_group_details = move.version_group_details.filter(
          (value) => {
            return this.pokemonVersionService.matchesDisplayVersion(
          ...
          move.version_group_details.filter(
            (detail) =>
              ...
              detail.move_learn_method.name ===
                PokemonMovesComponent.LEVEL_UP_METHOD
          ).length > 0
      )
      .sort((a, b) =>
      ...
      .filter(
        (move) =>
          move.version_group_details.filter(
            (detail) =>
              detail.move_learn_method.name ===
              PokemonMovesComponent.MACHINE_METHOD
          ).length > 0
      )
      ...
      .filter(
        (move) =>
          move.version_group_details.filter(
            (detail) =>
              detail.move_learn_method.name ===
              PokemonMovesComponent.TUTOR_METHOD
          ).length > 0
      )
      .sort((a, b) => (a.move.name < b.move.name ? -1 : 1));
    ...
    this.eggMoves = this.versionFilteredMoves
      .filter(
        (move) =>
          move.version_group_details.filter(
            (detail) =>
              detail.move_learn_method.name === PokemonMovesComponent.EGG_METHOD
          ).length > 0
      )
      .sort((a, b) => (a.move.name < b.move.name ? -1 : 1));
  }
